...
+ (NSString *)_rawSystemInfoString {
    // Check if the hardware is simulator. If it cannot find required environment variables - fallback to normal code.
    if (TARGET_OS_SIMULATOR || TARGET_IPHONE_SIMULATOR) {
        NSDictionary<NSString *, NSString *> * environment = [[NSProcessInfo processInfo] environment];
        // Various XCode versions may use different env variable names. So try one, then other.
        NSString *deviceID = [environment objectForKey:@"SIMULATOR_DEVICE_NAME"];
        if (deviceID != nil && [deviceID length] > 0) {
            return deviceID;
        }
        deviceID = [environment objectForKey:@"SIMULATOR_MODEL_IDENTIFIER"];
        if (deviceID != nil && [deviceID length] > 0) {
            return deviceID;
        }
    }
